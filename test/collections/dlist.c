...
static int dlist_test_empty (void) {
	...
	clibs_log_raw ("dlist_is_empty () & dlist_is_not_empty ()\n");

	DoubleList *dlist = dlist_init (integer_delete, integer_comparator);

	if (dlist_is_empty (dlist)) clibs_log_raw ("BEFORE - List is empty\n");
	if (dlist_is_not_empty (dlist)) clibs_log_raw ("BEFORE - List is not empty\n");
	...
	if (dlist_is_empty (dlist)) clibs_log_raw ("AFTER - List is empty\n");
	if (dlist_is_not_empty (dlist)) clibs_log_raw ("AFTER - List is not empty\n");

	for (ListElement *le = dlist_start (dlist); le; le = le->next) {
		clibs_log_raw ("%4d", ((Integer *) le->data)->value);
	}
...
static int dlist_test_delete_if_empty (void) {
	...
	clibs_log_raw ("dlist_delete_if_empty ()\n");
...
static int dlist_test_insert_before_at_start (void) {
	...
	clibs_log_raw ("dlist_insert_before () at START\n");
...
static int dlist_test_insert_before_at_end (void) {
	...
	clibs_log_raw ("dlist_insert_before () at END\n");
...
static int dlist_test_insert_before_at_middle (void) {
	...
	clibs_log_raw ("dlist_insert_before () at MIDDLE\n");
...
static int dlist_test_insert_before_at_start_unsafe (void) {
	...
	clibs_log_raw ("dlist_insert_before () at START UNSAFE\n");
...
static int dlist_test_insert_before_at_end_unsafe (void) {
	...
	clibs_log_raw ("dlist_insert_before () at END UNSAFE\n");
...
static int dlist_test_insert_after_at_start (void) {
	...
	clibs_log_raw ("dlist_insert_after () at START\n");
...
static int dlist_test_insert_after_at_end (void) {
	...
	clibs_log_raw ("dlist_insert_after () at END\n");
...
static int dlist_test_insert_after_unsafe_at_start (void) {
	...
	clibs_log_raw ("dlist_insert_after_unsafe () at START\n");
...
static int dlist_test_insert_after_unsafe_at_end (void) {
	...
	clibs_log_raw ("dlist_insert_after_unsafe () at END\n");
...
static int dlist_test_insert_at_start (void) {
	...
	clibs_log_raw ("dlist_insert_at_start ()\n");
...
static int dlist_test_insert_at_start_unsafe (void) {
	...
	clibs_log_raw ("dlist_insert_at_start_unsafe ()\n");
...
static int dlist_test_insert_at_end (void) {
	...
	clibs_log_raw ("dlist_insert_at_end ()\n");
...
static int dlist_test_insert_at_end_unsafe (void) {
	...
	clibs_log_raw ("dlist_insert_at_end_unsafe ()\n");
...
static int dlist_test_insert_in_order (void) {
	...
	clibs_log_raw ("dlist_test_insert_in_order ()\n");
...
static int dlist_test_remove (void) {
	...
	clibs_log_raw ("dlist_remove () dlist size after removed all: %ld", dlist->size);
...
static int dlist_test_remove_start (void) {
	...
	clibs_log_raw ("dlist_remove_start ()\n");
	...
	clibs_log_raw ("dlist_remove_start () dlist size after removed all: %ld", dlist->size);
...
static int dlist_test_remove_start_unsafe (void) {
	...
	clibs_log_raw ("dlist_remove_start_unsafe ()\n");
	...
	clibs_log_raw ("dlist_remove_start_unsafe () dlist size after removed all: %ld", dlist->size);
...
static int dlist_test_remove_end (void) {
	...
	clibs_log_raw ("dlist_remove_end ()\n");
	...
	clibs_log_raw ("dlist_remove_end () dlist size after removed all: %ld", dlist->size);
...
static int dlist_test_remove_end_unsafe (void) {
	...
	clibs_log_raw ("dlist_remove_end_unsafe ()\n");
	...
	clibs_log_raw ("dlist_remove_end_unsafe () dlist size after removed all: %ld", dlist->size);
...
static int dlist_test_remove_by_condition (void) {
	...
	clibs_log_raw ("dlist_remove_by_condition ()\n");

	DoubleList *list = dlist_init (integer_delete, integer_comparator);

	clibs_log_raw ("Insert 10 numbers:\n");
...
static int dlist_test_remove_at (void) {
	...
	clibs_log_raw ("Removed at 2 & 7 -> dlist size: %ld\n", dlist->size);
	for (ListElement *le = dlist_start (dlist); le; le = le->next) {
		clibs_log_raw ("%4d", ((Integer *) le->data)->value);
	}
...
// 	clibs_log_raw ("test_insert_and_remove ()\n");
...
// 	clibs_log_raw ("\n\n");
// 	clibs_log_raw ("Remove 2 from the start:\n");
// 	for (int i = 0; i < 2; i++) {
...
// 	clibs_log_raw ("Remove 2 from the start again:\n");
// 	for (int i = 0; i < 2; i++) {
// 		free (dlist_remove_element (list, NULL));
// 	}
...
// 	clibs_log_raw ("test_insert_and_remove_unsafe ()\n\n");
...
static void test_traverse_method (void *list_element_data, void *method_args) {
	...
	clibs_log_raw ("%4d", ((Integer *) list_element_data)->value);
...
static int dlist_test_traverse_threads (void) {
	...
	clibs_log_raw ("dlist_traverse () THREADS\n");
...
static int dlist_test_insert_threads (void) {
	...
	clibs_log_raw ("dlist_test_insert_threads ()\n");
	...
	clibs_log_raw ("Elements in dlist after 4 x 10 inserted: %ld\n\n", dlist->size);
...
// 	clibs_log_raw ("dlist_test_remove_threads ()\n");
...
// 	clibs_log_raw ("Elements in dlist after 4 x 10 removed: %ld\n\n", dlist->size);
...
static int dlist_test_to_array (void) {
	...
	if (array) {
		...
		for (size_t idx = 0; idx < count; idx++) {
			clibs_log_raw ("%4d", ((Integer *) array[idx])->value);
		}
...
static int dlist_test_split_half (void) {
	...
	clibs_log_raw ("dlist_split_half ()\n");
	...
	clibs_log_raw ("SECOND half: %ld", split->size);
	for (ListElement *le = dlist_start (split); le; le = le->next) {
		clibs_log_raw ("%4d", ((Integer *) le->data)->value);
	}
...
static int dlist_test_split_by_condition (void) {
	...
	clibs_log_raw ("dlist_split_by_condition ()\n");
...
static int dlist_test_merge_two_by_condition (void) {
	...
	clibs_log_raw ("dlist_merge_two_by_condition ()\n");
...
static int dlist_test_merge_many (void) {
	...
	clibs_log_raw ("dlist_merge_many ()\n");
	...
	clibs_log_raw ("One (%ld)\n", one->size);
	clibs_log_raw ("Two (%ld)\n", two->size);
	clibs_log_raw ("Three (%ld)\n", three->size);
	clibs_log_raw ("Four (%ld)\n", four->size);
